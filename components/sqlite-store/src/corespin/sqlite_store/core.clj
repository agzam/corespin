...
(def default-db-file "./investigation.sqlite3")
       ...
       (jdbc/with-transaction [tx db]
         (let [feed-ct (->> {:insert-or-ignore-into :feed :values [feed]}
           ...
           (doseq [indicator indicators]
             (let [ind-ct (->> {:insert-or-ignore-into :indicator
                                :values [indicator]}
...
(defn get-indicators
  ...
  ([params]
   (let [db (init-ds default-db-file)]
     (get-indicators params db)))
  ([{:keys [_type _tlp _indicator
            limit offset] :as params}
    db]
   (let [q (cond-> {:select
                    [:f.* :i.*
                     ;; TODO: figure out :raw in nested queries
             ...
             true (sql/format))]
     (query db q {:builder-fn rs/as-unqualified-kebab-maps}))))
